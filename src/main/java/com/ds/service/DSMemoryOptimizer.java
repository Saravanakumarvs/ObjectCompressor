...
public class DSMemoryOptimizer implements DataTypeMemoryOptimizer {
	...
	public DSMemoryOptimizer(
			Map<Class<? extends Object>, DataTypeOptimizer> optimizers,
			Logger logger) {
		this.logger = logger;
	...
	public void optimize(Object obj, Context context) {
		...
		for (Field field : fields) {
			...
			try {
				...
				logger.log(Level.SEVERE, "Optimization of field failed :"
						+ field, t);
			}
		}
	}

	private void optimize(Object obj, Field field, Context context)
		...
		if (null != value && false == context.contains(value)) {
			...
			DataTypeOptimizer optimizer = optimizers.get(klass);
			if (null != optimizer) {
				Object optimizedValue = optimizer.optimize(field, value,
						context);
	...
	private static boolean needsToSkipField(Field field) {
		return Modifier.isFinal(field.getModifiers())
				...
				|| field.isAnnotationPresent(ExcludeDataTypeOptimizer.class);
	}
